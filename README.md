...
## 통행료 OCR
- Tesseract(`pytesseract`) 무료·오픈소스 OCR을 유지합니다.
- 이미지 비율만으로 강제 분할하지 않고, 긴 세로 구분선 또는 실제 빈 여백을 먼저 찾아 영수증 영역을 분리합니다.
- 구분선이 불확실하면 전체 이미지와 비교하고, 마지막 수단으로만 2·3등분 보조 분할을 사용합니다.
- **보고서 인쇄 빠른 경로**: 영수증 구분선/여백을 먼저 나누고 PSM 11 한 번으로 판독하며, 확정되지 않은 영수증만 PSM 4를 1회 추가합니다.
- 일반 캡처보다 하이패스 **보고서 인쇄 화면 캡처를 권장**합니다.
- 공급가액+부가세(A), 차종 총액(B), KEC/CNE 합계(C)를 여러 OCR 판독 결과에서 모아 교차검증합니다.
- 동일 금액을 여러 OCR 패스에서 읽어도 한 종류의 의미 패턴뿐이면 자동 확정하지 않고 확인 필요 상태를 유지합니다.
- Cloud Run 로그의 `[TOLL_OCR]` 항목에서 선택된 분할방식, 인식건수, 확정건수, 합계를 확인할 수 있습니다.
